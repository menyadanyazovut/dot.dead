...
const Scene3D = (() => {
  ...
  function build(graves) {
    ...
    for (let i = 0; i < 11; i++) {
      ...
      sp.userData.baseOpacity = mat.opacity; // remembered so the fog can fade them
      const w = 34 + crand() * 30;
    ...
    // rain dims the cloud field along with the sky
    const cloudBase = new THREE.Color(COLORS.cloud);
    function setCloudShade(mul) {
      for (const sp of clouds) sp.material.color.copy(cloudBase).multiplyScalar(mul);
    }
    // the finale fog fades the clouds out entirely (1 = normal, 0 = gone)
    function setCloudFade(mul) {
      for (const sp of clouds) sp.material.opacity = sp.userData.baseOpacity * mul;
    }
    ...
    return { scene, update, collidersNear, sitesNear, landmarks, setCloudShade, setCloudFade, groundHeight: Terrain.walkHeight, CHUNK };
  }

// The pocket compass. Press E to draw it from below the screen; it floats
...
// walking barely stirs it. E again puts it away.
...
const Compass = (() => {
  ...
  function create(camera, world) {
    ...
    let suppressed = false; // wading in a lake: the compass stays pocketed
    ...
    function setSuppressed(v) {
      ...
      suppressed = v;
      if (v && deployed) {
        deployed = false; // it slides back into the pocket on its own
        vsx -= 0.5;
      }
    }
    ...
    return { toggle, impulse, update, setSuppressed, isDeployed: () => deployed };
  }
